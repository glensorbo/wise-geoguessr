...
| `signAuthToken.ts`        | `signAuthToken`        | Signs a short-lived (15m) auth JWT with `role` and `name` claims              |
| `verifyToken.ts`          | `verifyToken`          | Verifies and decodes a JWT; extracts `role` (defaults to `'user'`) and `name` |
