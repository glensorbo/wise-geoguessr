...
| `resultsPage.tsx`       | `/results`          | Points DataGrid (player column headers show `PlayerSparklineHeader` with last-5 score sparkline; date column shows 🔗 link icon when a `gameLink` is attached) + Weekly LineChart + year selector, with dense phone-friendly table and chart handling |
| `statsPage.tsx`         | `/stats`            | Won vs Played, Points Per Round, Accumulated Points, Championship Standings charts + year selector; Championship Standings uses an inverted Y-axis (rank 1 at top), with small-screen chart polish                                                    |
